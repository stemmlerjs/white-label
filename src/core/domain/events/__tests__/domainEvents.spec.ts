...
describe('Domain Events', () => {
  ...
  describe('Given a JobCreatedEvent, JobDeletedEvent and a PostToSocial handler class', () => {
    it('Should be able to setup event subscriptions', () => {
      ...
      expect(Object.keys(DomainEvents['handlersMap']).length).to.not.eq(0);
      expect(Object.keys(DomainEvents['handlersMap']).length).to.eq(2);
    ...
    it('Should add the event to the DomainEvents list when the event is created', () => {
      ...
      setTimeout(() => {
        expect(domainEventsAggregateSpy.calledOnce).to.be.true;
        expect(domainEventsAggregateSpy.callCount).to.be.eq(0)
        expect(DomainEvents['markedAggregates'][0]['length']).to.be.eq(1);
      }, 1000);
    });
    ...
    it('Should call the handlers when the event is dispatched after marking the aggregate root', () => {
      ...
      setTimeout(() => {
        expect(jobCreatedEventSpy.calledOnce).to.be.false;
        expect(jobDeletedEventSpy.calledOnce).to.be.true;
      }, 1000);
    });
